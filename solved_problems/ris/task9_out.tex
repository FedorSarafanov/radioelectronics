...
% \pgfplotstableread{data.tsv}\mytable
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			%
			xlabel={$\omega$}, 			% ПодПись оси Y
			ylabel={$|S(w)|$}, 			% ПодПись оси X
			%
			major grid style={
		...
		\addplot[domain={-20:20},samples=700] plot {abs(sin(deg(x/3))/(x/3)*cos(deg(x)))};% node[pos=.5, above]{$\sqrt\pi A\tau$};% node[pos=.8, below]{$\tau$}; 
		\addplot[dotted,domain={-20:20},samples=700] plot {abs(sin(deg(x/3))/(x/3)))};% node[pos=.5, above]{$\sqrt\pi A\tau$};% node[pos=.8, below]{$\tau$}; 
